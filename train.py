...
def maybe_download_and_extract():
    ...
    tarfile.open(filepath, "r:gz").extractall(dest_directory, filter="data")
...
def add_input_distortions(
    ...
    jpeg_data = tf.compat.v1.placeholder(tf.compat.v1.string, name="DistortJPGInput")
    decoded_image = tf.compat.v1.image.decode_jpeg(
        jpeg_data, channels=MODEL_INPUT_DEPTH
    )
    ...
    precrop_shape_as_int = tf.compat.v1.cast(precrop_shape, dtype=tf.compat.v1.int32)
    precropped_image = tf.compat.v1.image.resize_bilinear(
        decoded_image_4d, precrop_shape_as_int
    )
    precropped_image_3d = tf.compat.v1.squeeze(precropped_image, squeeze_dims=[0])
    cropped_image = tf.compat.v1.random_crop(
    ...
    with tf.compat.v1.Session(graph=graph) as sess:
        ...
        for i in range(FLAGS.how_many_training_steps):
            ...
            if (i % FLAGS.eval_step_interval) == 0 or is_last_step:
                ...
                print(
                    "Step: %d, Train accuracy: %.4f%%, Cross entropy: %f, Validation accuracy: %.1f%% (N=%d)"
                    % (
                        i,
                        train_accuracy * 100,
                        cross_entropy_value,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_dir", type=str, default="", help="Path to folders of labeled images."
    )
    parser.add_argument(
        "--output_graph",
        type=str,
